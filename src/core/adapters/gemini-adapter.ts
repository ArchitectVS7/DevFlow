...
/**
 * Gemini CLI adapter
 * Commands stored as TOML files under .gemini/commands/devflow by default
 */
export class GeminiAdapter extends BaseAdapter {
  ...
  get directory(): string {
    ...
    return useNamespace ? path.join('.gemini', 'commands', 'devflow') : path.join('.gemini', 'commands');
  }
  ...
  getTargetFilename(name: string): string {
    ...
    const namespaced = commandPath.endsWith(path.join('commands', 'devflow'));
    const baseName = namespaced ? name : `devflow-${name}`;
