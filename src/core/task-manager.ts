...
export class TaskManager {
  ...
  private async writeTasksFile(
    ...
    // Add footer
    content += '---\n\n';
    content += '*Generated by DevFlow /devflow:plan*\n';
  ...
  async findPrdDirectory(projectName?: string): Promise<string> {
    const baseDir = '.devflow/outputs';

    if (!await fs.pathExists(baseDir)) {
      throw new Error('No .devflow/outputs directory found. Have you generated a PRD yet?');
    }
    ...
    if (prdDirs.length === 0) {
      throw new Error('No PRD directories found in .devflow/outputs');
    }
