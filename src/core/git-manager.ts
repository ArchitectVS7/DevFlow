...
export class GitManager {
  ...
  private generateCommitMessage(options: CommitOptions): string {
    ...
    // Header
    if (phase) {
      message += `devflow: ${phase}\n\n`;
    } else if (description) {
      message += `devflow: ${description}\n\n`;
    } else if (tasks.length === 1) {
      message += `devflow: ${tasks[0]}\n\n`;
    } else if (tasks.length > 1) {
      message += `devflow: implement ${tasks.length} tasks\n\n`;
    } else {
      message += `devflow: task completed\n\n`;
    }
    ...
    message += 'Generated by DevFlow /devflow:implement';
