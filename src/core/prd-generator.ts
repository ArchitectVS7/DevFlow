...
export class PrdGenerator {
  ...
  private readonly defaultOutputDir = '.devflow/outputs';
  ...
  async generate(
    ...
    const metadata: PrdMetadata = {
      ...
      generatedBy: 'DevFlow PRD Generator',
      projectName,
      version: '1.0.0',
    };
  ...
  private async generateFullPrd(data: Record<string, unknown>, outputPath: string): Promise<void> {
    ...
    const customTemplatePath = '.devflow/templates/full-prd-template.hbs';
  ...
  private async generateQuickPrd(data: Record<string, unknown>, outputPath: string): Promise<void> {
    ...
    // Check if custom template exists
    const customTemplatePath = '.devflow/templates/quick-prd-template.hbs';
    const templateToUse = await fs.pathExists(customTemplatePath)
      ? customTemplatePath
      : templatePath;
