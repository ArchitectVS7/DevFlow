/**
 * ConfigManager - Manages .devflow-implement-config.json
 *
 * This class handles:
...
export interface ImplementConfig {
  ...
  /** Map of task IDs to GitHub Issue IDs */
...
export class ConfigManager {
   ...
   * @param configPath - Path to config file (.devflow-implement-config.json)
   * @returns Implementation configuration
   */
  async read(configPath: string): Promise<ImplementConfig> {
    if (!(await fs.pathExists(configPath))) {
      throw new Error(`Config file not found: ${configPath}\n\nHint: Run "devflow implement" first to initialize configuration`);
    }
  ...
  static getConfigPath(prdPath: string): string {
    return path.join(prdPath, '.devflow-implement-config.json');
  }
}
