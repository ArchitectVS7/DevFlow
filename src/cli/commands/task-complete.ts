...
export default class TaskComplete extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (!result.success) {
        ...
        console.log(chalk.gray('  2. Verify task ID matches exactly (run "devflow implement" to see current task)'));
      ...
      // GitHub Integration
      const globalConfigPath = path.join(process.cwd(), '.devflow', 'config.json');
      const githubManager = new GitHubManager();
      await githubManager.initialize(globalConfigPath);

      if (githubManager.isConfigured()) {
        const issueId = config.githubIssueIds?.[taskId];
        if (issueId) {
          console.log(chalk.dim(`Updating GitHub Issue #${issueId}...`));
          try {
            await githubManager.updateIssueStatus(parseInt(issueId, 10), 'COMPLETE');
            console.log(chalk.green('✓ GitHub Issue updated'));
          } catch (error) {
            console.log(chalk.yellow(`⚠ Failed to update GitHub Issue: ${(error as Error).message}`));
          }
        }
      }
  ...
  private async findConfigFile(providedPath?: string): Promise<string> {
    ...
    // Auto-discover: Look for .devflow/outputs/*/.devflow-implement-config.json
    const outputsDir = path.join(process.cwd(), '.devflow', 'outputs');

    if (!(await fs.pathExists(outputsDir))) {
      throw new Error('No .devflow/outputs directory found.\n\nHint: Run "devflow implement" first to initialize');
    }
    ...
    for (const entry of entries) {
      ...
      const configPath = path.join(outputsDir, entry.name, '.devflow-implement-config.json');
  ...
  private async handleGitCommit(
    ...
    switch (strategy) {
      case 'per-task':
        shouldCommit = true;
        commitMessage = `devflow: ${completedTask.description}`;
        break;
      ...
      case 'per-5-tasks':
        ...
        if (shouldCommit) {
          ...
          commitMessage = `devflow: Completed ${last5.length} tasks\n\nCompleted tasks:\n${taskDescriptions.map(d => `- ${d}`).join('\n')}`;
        }
        break;
      ...
      case 'per-phase': {
        ...
        if (currentPhase) {
          ...
          if (shouldCommit) {
            commitMessage = `devflow: Completed ${currentPhase.name}\n\nCompleted tasks:\n${currentPhase.tasks.map((t: Task) => `- ${t.description}`).join('\n')}`;
          }
        }
        break;
      }
    ...
    const fullMessage = `${commitMessage}\n\nGenerated by DevFlow task-complete`;
  ...
  private async showNextTask(taskManager: TaskManager, phases: TaskPhase[]): Promise<void> {
    ...
    if (!nextTask) {
      ...
      console.log(chalk.dim('Hint: Run "devflow archive" to archive this project\n'));
      return;
    }
