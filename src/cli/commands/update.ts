...
export default class Update extends Command {
  ...
  async run(): Promise<void> {
    ...
    this.log(chalk.bold.cyan('🔄 Updating DevFlow integration...\n'));
  ...
  private getAgentsContent(): string {
    return `## DevFlow Integration

This project uses DevFlow for prompt improvement and PRD generation.

### Available Commands
- \`devflow prd\` - Generate a comprehensive PRD through guided questions
- \`devflow fast [prompt]\` - Quick prompt improvements with smart triage
- \`devflow deep [prompt]\` - Comprehensive prompt analysis
- \`devflow start\` - Start a conversational session for iterative development
- \`devflow summarize\` - Extract requirements from conversation history
- \`devflow list\` - List all sessions and outputs
- \`devflow show [session-id]\` - Show detailed session information
...
devflow fast "Build a user auth system"

# Deep prompt analysis
devflow deep "Build a user auth system"

# Start conversational mode
devflow start
\`\`\`

Learn more: https://github.com/Bob5k/DevFlow`;
  }
...
### /devflow:fast [prompt]
Quick prompt improvements with smart triage. DevFlow will analyze your prompt and recommend deep analysis if needed. Perfect for making "shitty prompts good" quickly.
...
### /devflow:prd
Launch the PRD generation workflow. DevFlow will guide you through strategic questions and generate both a comprehensive PRD and a quick-reference version optimized for AI consumption.

### /devflow:start
Enter conversational mode for iterative prompt development. Discuss your requirements naturally, and later use \`/devflow:summarize\` to extract an optimized prompt.

### /devflow:summarize
Analyze the current conversation and extract key requirements into a structured prompt and mini-PRD.

**When to use which mode:**
- **Fast mode** (\`/devflow:fast\`): Quick cleanup for simple prompts
- **Deep mode** (\`/devflow:deep\`): Comprehensive analysis for complex requirements
