...
export default class Execute extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (allPrompts.length === 0) {
        ...
        console.log(chalk.cyan('  /devflow:fast "your requirement"'));
        console.log(chalk.cyan('  /devflow:deep "your requirement"'));
        console.log();
        return;
      }
      ...
      if (flags.id) {
        ...
        if (!selectedPrompt) {
          ...
          console.log(chalk.cyan('Run devflow prompts list to see available prompts'));
          console.log();
          return;
        }
      }
      // Auto-select latest with optional filtering
        ...
        if (filtered.length === 0) {
          ...
          console.log(chalk.cyan(`  /devflow:${source === 'any' ? 'fast' : source} "your requirement"`));
          console.log();
          return;
        }
  ...
  private async executePrompt(prompt: PromptMetadata, manager: PromptManager): Promise<void> {
    ...
    if (stats.executedPrompts >= 5) {
      ...
      console.log(chalk.yellow(`   Run /devflow:prompts clear to clean up.`));
      console.log();
    }
