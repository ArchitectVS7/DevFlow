...
import { DevFlowConfig, DEFAULT_CONFIG } from '../../types/config.js';
...
export default class Init extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (await FileSystem.exists('.devflow')) {
        const { reinit } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'reinit',
            message: 'DevFlow is already initialized. Reinitialize?',
            default: false,
          },
        ]);
      ...
      const { selectedProviders } = await inquirer.prompt([
        {
          ...
          choices: [
            ...
            {
              name: 'Augment CLI (.augment/commands/devflow/)',
              value: 'augment',
            },
            {
            ...
            {
              name: 'Crush CLI (.crush/commands/devflow/)',
              value: 'crush',
            },
            {
              name: 'Claude Code (.claude/commands/devflow/)',
              value: 'claude-code',
            },
            {
            ...
            {
              name: 'Gemini CLI (.gemini/commands/devflow/)',
              value: 'gemini',
            },
            {
            ...
            {
              name: 'Qwen Code (.qwen/commands/devflow/)',
              value: 'qwen',
            },
      ...
      for (const providerName of selectedProviders) {
        ...
        if (adapter.name === 'gemini' || adapter.name === 'qwen') {
          const defaultNamespacePath = path.join(`.${adapter.name}`, 'commands', 'devflow');
          const { useNamespace } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'useNamespace',
              message: `Store ${adapter.displayName} commands under ${defaultNamespacePath}? (Produces /devflow:<command> shortcuts)`,
              default: true,
            },
          ]);
        ...
        if (adapter.name === 'gemini' || adapter.name === 'qwen') {
          ...
          const isNamespaced = commandPath.endsWith(path.join('commands', 'devflow'));
  ...
  private async generateConfig(providers: string[]): Promise<void> {
    ...
    const configPath = '.devflow/config.json';
  ...
  private async generateInstructions(): Promise<void> {
    const instructions = `# DevFlow Instructions

Welcome to DevFlow! This directory contains your local DevFlow configuration and data.

## Directory Structure

\`\`\`
.devflow/
├── config.json           # Your DevFlow configuration
...
### Prompt Improvement
- \`devflow fast "<prompt>"\` - Quick CLEAR (C/L/E) improvements with smart triage
- \`devflow deep "<prompt>"\` - Comprehensive CLEAR (C/L/E/A/R) analysis with alternatives
- \`devflow execute [--latest]\` - Execute saved prompts from fast/deep optimization
- \`devflow prompts list\` - View all saved prompts with status (NEW/EXECUTED/OLD/STALE)
- \`devflow prompts clear\` - Cleanup prompts (\`--executed\`, \`--stale\`, \`--fast\`, \`--deep\`, \`--all\`)

### PRD & Planning
- \`devflow prd\` - Generate PRD through guided Socratic questions
- \`devflow plan\` - Transform PRD or session into phase-based \`tasks.md\`
- \`devflow start\` - Start conversational mode for requirements gathering
- \`devflow summarize [session-id]\` - Extract mini-PRD and prompts from conversation

### Implementation
- \`devflow implement [--commit-strategy=<type>]\` - Execute tasks with optional git auto-commits
- \`devflow task-complete <taskId>\` - Mark task complete with validation, auto-show next task

### Project Management
- \`devflow list [--sessions|--outputs]\` - List sessions and/or output projects
- \`devflow show [session-id|--output <project>]\` - Inspect session or project details
- \`devflow archive [project] [--restore]\` - Archive completed projects or restore them

### Configuration
- \`devflow init\` - Initialize DevFlow (you just ran this!)
- \`devflow config [get|set|edit|reset]\` - Manage configuration preferences
- \`devflow update [--docs-only|--commands-only]\` - Refresh managed docs and slash commands
...
### Prompt Improvement
- \`/devflow:fast [prompt]\` - Quick prompt improvements
- \`/devflow:deep [prompt]\` - Comprehensive prompt analysis
- \`/devflow:execute\` - Execute saved prompts
- \`/devflow:prompts\` - Manage prompt lifecycle

### PRD & Planning
- \`/devflow:prd\` - Generate PRD through guided questions
- \`/devflow:plan\` - Generate task breakdown from PRD
- \`/devflow:start\` - Start conversational mode
- \`/devflow:summarize\` - Summarize conversation

### Implementation
- \`/devflow:implement\` - Execute task workflow with git integration
- \`/devflow:task-complete\` - Mark task as complete

### Project Management
- \`/devflow:archive\` - Archive completed projects
...
1. **Create improved prompt**:
   \`\`\`bash
   devflow fast "your prompt here"
   # or
   devflow deep "your prompt here"
   \`\`\`
   - CLI auto-saves to \`.devflow/outputs/prompts/fast/\` or \`deep/\`
   - Slash commands require manual save per template instructions

2. **Execute saved prompt**:
   \`\`\`bash
   devflow execute --latest  # Most recent prompt
   devflow execute           # Interactive selection
   \`\`\`

3. **Manage prompts**:
   \`\`\`bash
   devflow prompts list              # View all with status
   devflow prompts clear --executed  # Remove executed prompts
   devflow prompts clear --stale     # Remove stale (30+ days)
   \`\`\`
...
1. **Generate PRD**:
   \`\`\`bash
   devflow prd
   # Creates: .devflow/outputs/<project>/full-prd.md + quick-prd.md
   \`\`\`

2. **Create task breakdown**:
   \`\`\`bash
   devflow plan
   # Creates: .devflow/outputs/<project>/tasks.md
   \`\`\`
...
4. **Mark tasks complete**:
   \`\`\`bash
   devflow task-complete <taskId>
   # Validates completion, optionally commits, shows next task
   \`\`\`

5. **Archive when done**:
   \`\`\`bash
   devflow archive my-project
   \`\`\`

### Git Auto-Commit Strategies (v2.8.1)

When using \`devflow implement --commit-strategy=<type>\`:
...
\`\`\`bash
devflow fast "Add user authentication"
devflow execute --latest
\`\`\`

**Create and execute strategy**:
\`\`\`bash
devflow prd              # Generate PRD
devflow plan             # Create tasks.md
devflow implement        # Execute with manual commits
\`\`\`

**Capture conversation**:
\`\`\`bash
devflow start            # Record conversation
# ... discuss requirements ...
devflow summarize        # Extract mini-PRD + prompt
\`\`\`

**Stay organized**:
\`\`\`bash
devflow list             # See all projects
devflow show --output my-project
devflow archive my-project
\`\`\`

## Customization

Create custom templates in \`.devflow/templates/\` to override defaults:
...
Edit configuration:
\`\`\`bash
devflow config edit      # Opens config.json in $EDITOR
devflow config set key=value
\`\`\`

## Need Help?

- **Documentation**: https://github.com/DevFlowDev/DevFlow
- **Issues**: https://github.com/DevFlowDev/DevFlow/issues
- **Version**: Run \`devflow version\` to check your installed version
- **Update managed blocks**: Run \`devflow update\` to refresh documentation
`;

    await FileSystem.writeFileAtomic('.devflow/INSTRUCTIONS.md', instructions);
  }
  ...
  private async injectDocumentation(adapter: AgentAdapter): Promise<void> {
    ...
    await DocInjector.injectBlock('AGENTS.md', this.extractDevFlowBlock(agentsContent));

    // Inject CLAUDE.md if Claude Code selected
    if (adapter.name === 'claude-code') {
      const claudeContent = DocInjector.getDefaultClaudeContent();
      await DocInjector.injectBlock('CLAUDE.md', this.extractDevFlowBlock(claudeContent));
    }
  }
