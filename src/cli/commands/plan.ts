...
export default class Plan extends Command {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> --prd-path .devflow/outputs/my-project',
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      await githubManager.initialize(configPath);

      if (githubManager.isConfigured()) {
        console.log(chalk.dim('\nSyncing with GitHub...'));
        const configManager = new ConfigManager();
        const implementConfigPath = ConfigManager.getConfigPath(prdPath);

        // Initialize implement config if it doesn't exist
        if (!(await fs.pathExists(implementConfigPath))) {
          await configManager.write(implementConfigPath, {
            commitStrategy: 'none',
            tasksPath: path.join(prdPath, 'tasks.md'),
            currentTask: result.phases[0].tasks[0],
            stats: {
              total: result.totalTasks,
              completed: 0,
              remaining: result.totalTasks,
              percentage: 0,
            },
        ...
        const implementConfig = await configManager.read(implementConfigPath);
        if (!implementConfig.githubIssueIds) {
          implementConfig.githubIssueIds = {};
        }
        ...
        for (const phase of result.phases) {
          for (const task of phase.tasks) {
            ...
            if (implementConfig.githubIssueIds[task.id]) continue;

            try {
              const issue = await githubManager.createIssue({
                title: `[${task.id}] ${task.description}`,
                body: `**Phase**: ${task.phase}\n**PRD Reference**: ${task.prdReference || 'N/A'}\n\nGenerated by DevFlow.`,
                labels: ['devflow', 'task'],
              });

              implementConfig.githubIssueIds[task.id] = issue.id.toString();
              newIssuesCount++;

              // Move to NEW column
              await githubManager.updateIssueStatus(issue.id, 'NEW');

              process.stdout.write('.');
            } catch (error) {
              console.error(chalk.red(`\nFailed to create issue for ${task.id}: ${(error as Error).message}`));
            }
          }
        }

        await configManager.write(implementConfigPath, implementConfig);
        if (newIssuesCount > 0) {
          console.log(chalk.green(`\n✓ Created ${newIssuesCount} GitHub issues`));
        } else {
          console.log(chalk.dim('\nNo new GitHub issues created (all tasks linked)'));
        }
      }
      ...
      console.log(chalk.gray('  3. Run'), chalk.cyan('devflow implement'), chalk.gray('to start implementation'));
      console.log();
  ...
  private async prepareArtifactsFromSession(
    ...
    const outputsDir = path.join('.devflow', 'outputs', projectDirName);
  ...
  private async resolveProjectDirectory(
    ...
    const outputsDir = path.join(process.cwd(), '.devflow', 'outputs');
