...
export default class List extends Command {
  ...
  private async listSessions(projectFilter?: string, limit?: number): Promise<void> {
    ...
    if (sessions.length === 0) {
      ...
      this.log(chalk.gray('  Run ') + chalk.cyan('devflow start') + chalk.gray(' to create a new session.'));
      return;
    }
  ...
  private async listOutputs(projectFilter?: string, limit?: number, includeArchived = false): Promise<void> {
    ...
    if (!fs.existsSync(outputsDir)) {
      ...
      this.log(chalk.gray('  Run ') + chalk.cyan('devflow prd') + chalk.gray(' or ') + chalk.cyan('devflow summarize') + chalk.gray(' to generate outputs.'));
      return;
    }
  ...
  private async listArchivedOutputs(projectFilter?: string, limit?: number): Promise<void> {
    const archiveDir = path.join(process.cwd(), '.devflow', 'outputs', 'archive');
    ...
    this.log(chalk.gray(`  Use ${chalk.cyan('devflow archive --restore <project>')} to restore a project`));
  }
}
