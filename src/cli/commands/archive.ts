...
export default class Archive extends Command {
  ...
  private async interactiveArchive(archiveManager: ArchiveManager, yes: boolean): Promise<void> {
    ...
    if (archivableProjects.length === 0) {
      ...
      this.log(chalk.gray('Use ') + chalk.cyan('devflow list --outputs') + chalk.gray(' to see all projects.'));
      return;
    }
      ...
      const { confirm } = await inquirer.prompt([
        {
          type: 'confirm',
          name: 'confirm',
          message: `Archive "${selectedProject}"? (This will move it to .devflow/outputs/archive/)`,
          default: false,
        },
      ]);
  ...
  private async archiveSpecificProject(
    ...
    // Check task status
    const projectPath = `.devflow/outputs/${projectName}`;
  ...
  private async listArchivedProjects(archiveManager: ArchiveManager): Promise<void> {
    ...
    if (archivedProjects.length === 0) {
      ...
      this.log(chalk.gray('\nUse ') + chalk.cyan('devflow archive') + chalk.gray(' to archive completed projects.\n'));
      return;
    }
