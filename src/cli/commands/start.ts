...
export default class Start extends Command {
  ...
  private displayIntroduction(projectName: string, sessionId: string): void {
    ...
    console.log(chalk.gray('  • Use ') + chalk.cyan('/devflow:summarize') + chalk.gray(' later to extract structured requirements'));
    console.log();
  ...
  private async displayExitInfo(): Promise<void> {
    ...
    if (session) {
      ...
      const sessionPath = path.join('.devflow/sessions', `${session.id}.json`);
      console.log();
      console.log(chalk.bold('Session file:'));
      console.log(chalk.dim(`  ${sessionPath}`));
      console.log();
      console.log(chalk.bold('Next steps:'));
      console.log(chalk.gray('  • Resume: ') + chalk.cyan(`devflow start --resume ${session.id}`));
      console.log(chalk.gray('  • Summarize: ') + chalk.cyan(`devflow summarize ${session.id}`));
      console.log(chalk.gray('  • View: ') + chalk.cyan(`devflow show ${session.id}`));
      console.log();
    } else {
