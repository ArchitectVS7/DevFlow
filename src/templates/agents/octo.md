# DevFlow Instructions for Octofriend

This file provides DevFlow workflows optimized for Octofriend's capabilities including model switching, multi-LLM support, and thinking token management.
...
4. Save both to `.devflow/outputs/[project-name]/`:
...
4. Save to `.devflow/sessions/[timestamp].md` if filesystem access available
...
**Process:**
1. Locate the PRD file in `.devflow/outputs/[project-name]/PRD.md`
...
4. Save to `.devflow/outputs/[project-name]/tasks.md`
...
**Process:**
1. Load tasks from `.devflow/outputs/[project-name]/tasks.md`
...
When starting implementation with `devflow implement`:
...
3. Move project folder from `.devflow/outputs/[project-name]/` to `.devflow/outputs/archive/[project-name]/`
4. Confirm archival and show new location

**CLI Command:** Users with DevFlow CLI can run `devflow archive` for interactive archiving or `devflow archive [project-name]` for direct archiving.
...
**Process:**
1. List saved prompts: `devflow prompts list`
   - Shows all prompts with status (NEW, EXECUTED, OLD, STALE)
   - Displays age warnings and storage statistics
2. Execute interactively: `devflow execute` (select from list)
   - Or execute latest: `devflow execute --latest`
   - Or execute specific: `devflow execute --id <prompt-id>`
3. Prompt is marked as EXECUTED after display
4. Implement the requirements from the optimized prompt
5. Cleanup after completion: `devflow prompts clear --executed`
...
| Command | Use it for |
| --- | --- |
| `devflow init` | Rebuild `.devflow` structure and regenerate provider assets. |
| `devflow fast` / `devflow deep` | CLEAR-based prompt improvement (quick vs. comprehensive). CLI auto-saves prompts. Slash commands require manual saving per template instructions. |
| `devflow execute` | Execute saved prompts (interactive selection or `--latest` for most recent). |
| `devflow prompts list` | View saved prompts with lifecycle status (NEW, EXECUTED, OLD, STALE). |
| `devflow prompts clear` | Cleanup executed/stale prompts (`--executed`, `--stale`, `--fast`, `--deep`). |
| `devflow prd` | Guided questions to create PRDs. |
| `devflow plan` | Convert PRD artifacts into task lists. |
| `devflow implement [--commit-strategy=<type>]` | Start task execution (git: per-task, per-5-tasks, per-phase, none [default]). |
| `devflow task-complete <taskId>` | Mark task completed with validation and optional git commit. Auto-displays next task. |
| `devflow start` | Capture conversational requirements. |
| `devflow summarize` | Extract mini PRDs and prompts from sessions. |
| `devflow list` | List sessions or outputs (use `--sessions`, `--outputs`, `--archived`). |
| `devflow show` | Inspect sessions or output directories. |
| `devflow archive` | Archive or restore project outputs. |
| `devflow config` | View/update `.devflow/config.json`. |
| `devflow update` | Refresh documentation and slash commands. |
| `devflow version` | Display installed CLI version. |
...
- **Always save outputs** to `.devflow/outputs/` when possible
