...
# DevFlow Fast Mode - CLEAR Framework Quick Improvement

You are helping the user improve their prompt using DevFlow's fast mode, which applies the CLEAR Framework (Concise, Logical, Explicit components) with smart triage.
...
**Deep Mode Adds:** [A] Adaptive variations, [R] Reflective validation (use `/devflow:deep` for these)
...
3. **CLEAR-Aware Smart Triage**: Use multi-factor content-quality assessment to determine if deep analysis is needed:
   ...
   **Escalation Decision**:
   - If **Low CLEAR scores** + **2+ Secondary Indicators**: **Strongly recommend `/devflow:deep`**
   - If **Low CLEAR scores** only: **Suggest `/devflow:deep`** but can proceed with fast mode
...
**If user ran CLI command** (`devflow fast "prompt"`):
- Prompt is automatically saved ✓
- Skip to "Executing the Saved Prompt" section below

**If you are executing this slash command** (`/devflow:fast`):
- You MUST save the prompt manually
- Follow these steps:

#### Step 1: Create Directory Structure
```bash
mkdir -p .devflow/outputs/prompts/fast
```
...
- **Path**: `.devflow/outputs/prompts/fast/<prompt-id>.md`
...
Use the Write tool to update the index at `.devflow/outputs/prompts/fast/.index.json`:
...
Confirm:
- File exists at `.devflow/outputs/prompts/fast/<prompt-id>.md`
...
**Common workflows:**
- **Quick cleanup**: `/devflow:fast` → `/devflow:execute` → Implement
- **Review first**: `/devflow:fast` → `/devflow:prompts` → `/devflow:execute`
- **Need more depth**: `/devflow:fast` → (suggests) `/devflow:deep` → Comprehensive analysis
- **Strategic planning**: `/devflow:fast` → (suggests) `/devflow:prd` → Plan → Implement → Archive

**Related commands:**
- `/devflow:execute` - Execute saved prompt
- `/devflow:prompts` - Manage saved prompts
- `/devflow:deep` - Full CLEAR analysis (all 5 components: C, L, E, A, R)
- `/devflow:prd` - Generate PRD for strategic planning
- `/devflow:start` - Conversational exploration before prompting
...
- For comprehensive analysis with [A] and [R], recommend `/devflow:deep`
- For strategic planning, recommend `/devflow:prd`
...
```bash
echo '{"version":"1.0","prompts":[]}' > .devflow/outputs/prompts/fast/.index.json
```
...
- If output feels insufficient, escalate to `/devflow:deep`
...
**Solution**:
- Use `/devflow:deep` for Adaptive variations and Reflective validation
- OR use `/devflow:prd` if strategic planning is needed
