...
# DevFlow Archive - PRD Project Archival

> **⚠️ Agent Execution Note**: This command requires CLI execution. AI agents should run `devflow archive` via Bash tool to perform archival operations. Direct file manipulation is not recommended due to state tracking complexity.
...
2. **Choose the appropriate mode** based on user intent:
   ...
   - **Specific project**: `devflow archive [project-name]`
   - **Force archive incomplete**: `devflow archive [project-name] --force`
   - **Permanent delete**: `devflow archive [project-name] --delete`
   - **List archived**: `devflow archive --list`
   - **Restore**: `devflow archive --restore [project-name]`

3. **Before running the command**, validate:
   - Check if `.devflow/outputs/` directory exists
...
3. **Force Archive (Incomplete Tasks)**:
   ```bash
   devflow archive [project-name] --force
   ```
...
4. **Delete Project (Permanent Removal)**: **DESTRUCTIVE ACTION**
   ```bash
   devflow archive [project-name] --delete
   ```
...
6. **Restore from Archive**:
   ```bash
   devflow archive --restore [project-name]
   ```
...
- All files: PRD.md, PRD-quick.md, tasks.md, .devflow-implement-config.json
- Complete directory structure preserved

**Where it goes:**
- From: `.devflow/outputs/[project-name]/`
- To: `.devflow/outputs/archive/[project-name]/`

**What changes:**
- Archived projects won't show in `/devflow:plan` searches
- Archived projects won't show in `/devflow:implement` searches
- They're still accessible in archive directory
- Can be restored at any time

## Prompts Are Separate (v2.7+)

Fast/deep prompts from `/devflow:fast` and `/devflow:deep` are stored separately in `.devflow/outputs/prompts/`.

**Prompts are NOT archived with PRD projects.**

**Manage prompts separately:**
```bash
devflow prompts list           # View all prompts
devflow prompts clear          # Cleanup prompts
/devflow:prompts               # Full management workflow
```
...
Result: Project moved to .devflow/outputs/archive/user-authentication-system/
```
...
You: "I'll archive it with --force"
     ...
     Run: devflow archive old-api-design --force
...
You: "I'll restore it from the archive"
     ...
     Run: devflow archive --restore user-authentication-system

Result: Project moved back to .devflow/outputs/user-authentication-system/
```
...
You: "This will permanently delete the project. I'll run the delete command."
     ...
     Run: devflow archive api-experiment-1 --delete
...
Result: Project permanently deleted from .devflow/outputs/api-experiment-1/
```
...
2. **Check completion status first**:
   - Run `devflow archive` (interactive mode) to see archivable projects
...
3. **Execute the appropriate command**:
   - **Interactive selection**: `devflow archive` (let user pick from list)
   - **Specific project**: `devflow archive [project-name]`
   - **Force incomplete**: `devflow archive [project-name] --force`
   - **List archived**: `devflow archive --list`
   - **Restore**: `devflow archive --restore [project-name]`
   - **Delete**: `devflow archive [project-name] --delete` (with extra caution)

4. **Confirm before archiving**:
   - If using specific project mode, confirm project name with user
   - Mention the archive location (`.devflow/outputs/archive/`)
...
6. **Suggest restoration when appropriate**:
   ...
   - Run `devflow archive --list` to show what's archived
   - Offer to restore if needed via `devflow archive --restore [project]`
...
8. **After CLI execution**:
   ...
   - Mention next steps (e.g., "Project archived, you can restore with `/devflow:archive --restore`")
...
**Common workflows:**
- **Complete workflow**: `/devflow:implement` → [all tasks done] → `/devflow:archive` → Clean workspace
- **Review and archive**: `/devflow:archive` → [select completed project] → Archive
- **Restore old work**: `/devflow:archive --list` → `/devflow:archive --restore [project]` → Resume

**Related commands:**
- `/devflow:implement` - Complete remaining tasks before archiving
- `/devflow:plan` - Review task completion status
- `/devflow:prd` - Start new project after archiving old one
...
- Regular archiving improves `/devflow:plan` and `/devflow:implement` performance
- Use `--list` regularly to know what's been archived

## Troubleshooting

### Issue: No projects available to archive
**Cause**: No projects in `.devflow/outputs/` OR all already archived

**Agent recovery**:
1. Check if `.devflow/outputs/` exists: `ls .devflow/outputs/`
2. If directory doesn't exist: "No PRD projects found. Create one with `/devflow:prd`"
3. If empty: Run `devflow archive --list` to show archived projects
...
2. Ask user: "Project has X incomplete tasks. Do you want to:
   - Complete tasks first with `/devflow:implement`
   - Archive anyway with `--force` (tasks remain incomplete but archived)
   - Cancel archival"
3. If user confirms force: Run `devflow archive [project] --force`
...
2. Common fixes:
   - Check `.devflow/outputs/` exists and is writable
...
4. No recovery possible from DevFlow itself
...
3. If user truly concerned:
   - Review archive: `devflow archive --list`
   - Identify ancient/irrelevant projects
   - Delete only truly obsolete ones: `devflow archive [old-project] --delete`
...
1. Read the PRD to remind user:
   ```bash
   cat .devflow/outputs/archive/[project-name]/full-prd.md
   ```
