...
# DevFlow Implement - AI-Assisted Task Execution
...
1. **First-time setup - Run CLI command with optional git strategy**:
   ...
   Check if `.devflow-implement-config.json` exists in the PRD output folder.
   ...
   **If config file does NOT exist** (first time running implement):
   ...
   c. **This will**:
      ...
      - Create `.devflow-implement-config.json` file
...
2. **As the AI agent, you should**:
   ...
   a. **Read the configuration**:
      - Load `.devflow-implement-config.json` from the PRD folder
   ...
   e. **Complete the task programmatically**:
      ...
      - Instead, run: `devflow task-complete {task-id}`
      ...
      - Example: `devflow task-complete phase-1-setup-1`

   f. **Move to next task**:
      - The task-complete command shows the next task automatically
      - Read it and repeat the process
      - If you get interrupted, just run `devflow implement` again to resume

3. **Session Resume**:
   - When user runs `devflow implement` again, it automatically picks up from the last incomplete task
   - No manual tracking needed - the checkboxes in tasks.md are the source of truth

## Task Completion Workflow

**CRITICAL: Always use the `devflow task-complete` command**
...
# Example
devflow task-complete phase-1-setup-1
```
...
- Use `devflow task-complete {task-id}` after completing each task
- Reference the PRD for detailed requirements
- Ask for clarification when tasks are ambiguous
- Run `devflow implement` again if interrupted to resume
...
2. In the config file (`.devflow-implement-config.json`) under `currentTask.id`
3. When running `devflow implement` - shown next to task description
...
**Common workflows:**
- **Full workflow**: `/devflow:plan` → `/devflow:implement` → [execute all tasks] → `/devflow:archive`
- **Resume work**: `/devflow:implement` → Continue from last incomplete task
- **Iterative**: `/devflow:implement` → [complete task] → [pause] → `/devflow:implement` → [continue]

**Related commands:**
- `/devflow:plan` - Generate/regenerate task breakdown (previous step)
- `/devflow:archive` - Archive completed project (final step)
- `/devflow:prd` - Review PRD for context during implementation
...
### Issue: `.devflow-implement-config.json` not found
**Cause**: User hasn't run `devflow implement` CLI command first
**Solution** (inline):
- Error: "Config file not found. Run `devflow implement` first to initialize"
- CLI creates config and shows first task
- AI agent should wait for config before proceeding

### Issue: `devflow task-complete` command not found
**Cause**: DevFlow version doesn't have task-complete command OR not in PATH
**Solution**:
- Check DevFlow version: `devflow --version`
- Ensure DevFlow is up to date: `npm install -g devflow@latest`
- If issue persists, report bug to DevFlow maintainers
...
- Run `devflow task-complete` without arguments to see available tasks
...
**Solution**:
- Use `--force` flag: `devflow task-complete {task-id} --force`
- Or skip to next task shown by `devflow implement`
...
- Suggest `/devflow:archive` for completed project
