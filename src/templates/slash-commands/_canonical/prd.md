...
# DevFlow PRD Generation - CLEAR Framework Validated

You are helping the user create a Product Requirements Document (PRD) using DevFlow's Socratic questioning approach. **Generated PRDs are automatically validated using the CLEAR Framework** (Concise, Logical, Explicit) for AI consumption quality.
...
```bash
mkdir -p .devflow/outputs/{sanitized-project-name}
```

**Handle errors**:
- If directory creation fails: Check write permissions
- If `.devflow/` doesn't exist: Create it first: `mkdir -p .devflow/outputs/{project}`

### Step 3: Save Full PRD
**File path**: `.devflow/outputs/{project-name}/full-prd.md`
...
*Generated by DevFlow /devflow:prd*
*Generated: {ISO timestamp}*
```

### Step 4: Save Quick PRD
**File path**: `.devflow/outputs/{project-name}/quick-prd.md`
...
```bash
ls .devflow/outputs/{project-name}/
```
...
Files saved:
  • Full PRD: .devflow/outputs/{project-name}/full-prd.md
  • Quick PRD: .devflow/outputs/{project-name}/quick-prd.md
...
Next steps:
  ...
  • Run /devflow:plan to generate implementation tasks
```
...
**Note:** Adaptive (A) and Reflective (R) components are not applicable to PRDs. For prompt-level CLEAR analysis with all 5 components, use `/devflow:deep` instead.
...
**Common workflows:**
- **Full PRD workflow**: `/devflow:prd` → `/devflow:plan` → `/devflow:implement` → `/devflow:archive`
- **From deep mode**: `/devflow:deep` → (strategic scope detected) → `/devflow:prd`
- **Quick to strategic**: `/devflow:fast` → (realizes complexity) → `/devflow:prd`

**Related commands:**
- `/devflow:plan` - Generate task breakdown from PRD (next step)
- `/devflow:implement` - Execute tasks (after plan)
- `/devflow:summarize` - Alternative: Extract PRD from conversation instead of Q&A
...
- Consider suggesting `/devflow:start` for conversational exploration first
