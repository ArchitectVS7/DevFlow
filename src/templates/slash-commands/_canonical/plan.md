...
# DevFlow Plan - Task Breakdown Generator
...
1. **Validate prerequisites**:
   ...
   - **If not found**: Error inline - "No PRD found in `.devflow/outputs/`. Use `/devflow:prd` or `/devflow:summarize` first."
...
2. **Run the CLI command**:
   ...
   ```bash
   devflow plan --project project-name
   ```

   Or generate from saved session (auto-creates mini-prd.md):
   ```bash
   devflow plan --session SESSION_ID
   ```
...
1. **Read the PRD** from `.devflow/outputs/[project-name]/`
...
6. **Next steps**:
   ...
   - Then run `/devflow:implement` to start implementation
...
*Generated by DevFlow /devflow:plan*
```
...
**Path**: `.devflow/outputs/{project-name}/tasks.md`
...
**Common workflows:**
- **PRD workflow**: `/devflow:prd` → `/devflow:plan` → `/devflow:implement` → `/devflow:archive`
- **Conversation workflow**: `/devflow:summarize` → `/devflow:plan` → `/devflow:implement` → `/devflow:archive`
- **Standalone**: [Existing PRD] → `/devflow:plan` → Review tasks.md → `/devflow:implement`

**Related commands:**
- `/devflow:prd` - Generate PRD (typical previous step)
- `/devflow:summarize` - Extract mini-PRD from conversation (alternative previous step)
- `/devflow:implement` - Execute generated tasks (next step)
...
- Supports mini-PRD outputs from `/devflow:summarize` and session workflows via `--session` or `--active-session`
- You can manually edit tasks.md before implementing
- Use `--overwrite` flag to regenerate if needed

## Troubleshooting

### Issue: No PRD found in `.devflow/outputs/`
**Cause**: User hasn't generated a PRD yet

**Agent recovery**:
1. Check if `.devflow/outputs/` directory exists:
   ```bash
   ls .devflow/outputs/
   ```
2. If directory doesn't exist or is empty:
   ...
   - Suggest recovery options:
     - "Generate PRD with `/devflow:prd` for comprehensive planning"
     - "Extract mini-PRD from conversation with `/devflow:summarize`"
     - "Or use `devflow plan --session <id>` if you have a saved session"
...
3. Options for user:
   ...
   - **Regenerate**: Use `devflow plan --overwrite` after simplifying PRD
...
2. If PRD is vague:
   - Suggest: "Let's improve the PRD with `/devflow:deep` first"
   - Then regenerate tasks with `devflow plan --overwrite`
...
3. Options:
   ...
   - **Regenerate**: Use `devflow plan --overwrite` after fixing PRD
...
3. Decision tree:
   - **No progress** (all `[ ]`): Safe to use `devflow plan --overwrite`
...
4. If user confirms overwrite: Run `devflow plan --project {name} --overwrite`
...
2. Common fixes:
   ...
   - Check `.devflow/outputs/{project}/` has valid PRD
   - Verify project name is correct (no typos)
3. Try with explicit project: `devflow plan --project {exact-name}`
4. If persistent: Inform user to check DevFlow installation
