---
name: "DevFlow: Task Complete"
description: Mark a task as completed with validation and optional git commit
---

# DevFlow Task Complete - Mark Implementation Task Done
...
1. **Prerequisites**:
   - User must have run `devflow implement` to start implementation
   - `.devflow-implement-config.json` must exist in PRD folder
   - `tasks.md` must be generated from `devflow plan`
...
3. **Mark task as completed**:
   ...
   **Examples**:
   ```bash
   devflow task-complete phase-1-auth-1
   devflow task-complete phase-2-api-3
   devflow task-complete setup-1
   ```
...
5. **Git auto-commit behavior**:
   ...
   The command respects the git strategy configured in `devflow implement`:
   ...
   **Override for specific task**:
   ```bash
   devflow task-complete phase-1-auth-1 --no-git
   ```
...
6. **Handle command output**:
   ...
   Hint: Run "devflow archive" to archive this project
   ```
...
7. **After task completion**:
   ...
   b. **If all tasks complete**:
      ...
      - Suggest running `devflow archive` to archive the project
...
8. **Error recovery**:
   ...
   **If config file not found**:
   ...
   - Tell user to run `devflow implement` first
...
1. **Never manually edit tasks.md checkboxes** - Always use `devflow task-complete`
...
- `devflow implement` - Start implementation workflow (shows current task)
- `devflow plan` - Generate tasks.md from PRD
- `devflow archive` - Archive completed project (run after all tasks done)

## Next Steps

After completing all tasks:
1. Run `devflow archive` to archive the implementation
