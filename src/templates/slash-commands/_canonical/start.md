...
# DevFlow Conversational Mode - CLEAR Framework Enhanced

You are starting a DevFlow conversational session for iterative prompt and requirements development. **When complete, use `/devflow:summarize` to extract and CLEAR-optimize requirements** (Concise, Logical, Explicit).
...
1. Begin with a friendly introduction:
   ...
   When we're done, use /devflow:summarize to extract a CLEAR-optimized prompt from our conversation.
   ```
...
3. **Track conversation topics and manage complexity**:
   ...
   **Complexity indicators** (suggest wrapping up/summarizing):
   ...
   - Continue exploring
   - Use `/devflow:summarize` to extract what we have so far
   - Switch to `/devflow:prd` for more structured planning"
...
5. Remind them to use `/devflow:summarize` when ready to extract a CLEAR-optimized prompt.

## CLEAR Framework Connection

After the conversational session, `/devflow:summarize` will:
...
**Common workflows:**
- **Exploration to prompt**: `/devflow:start` → [conversation] → `/devflow:summarize` → CLEAR-optimized prompt
- **Exploration to PRD**: `/devflow:start` → [conversation] → `/devflow:prd` (answer questions with discussed info)
- **Exploration to planning**: `/devflow:start` → `/devflow:summarize` → `/devflow:plan` → Implement

**Related commands:**
- `/devflow:summarize` - Extract and CLEAR-optimize conversation (typical next step)
- `/devflow:prd` - Switch to structured PRD generation
- `/devflow:fast` or `/devflow:deep` - Direct prompt improvement instead of conversation
...
- Or suggest `/devflow:summarize` to extract what's been discussed
...
- Consider suggesting `/devflow:prd` for each topic separately

### Issue: Conversation exceeds 20 exchanges without clarity
**Cause**: Too exploratory without convergence
**Solution**:
- Suggest wrapping up: "We've covered a lot. Ready to `/devflow:summarize`?"
- Or pivot to `/devflow:prd` for structured planning
