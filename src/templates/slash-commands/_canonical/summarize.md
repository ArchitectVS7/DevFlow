---
name: "DevFlow: Summarize"
description: Extract and CLEAR-optimize requirements from conversation
---

# DevFlow Conversation Summarization - CLEAR Framework Enhanced
...
5. Suggest saving to `.devflow/outputs/[session-name]/`
...
**Common workflows:**
- **Standard flow**: `/devflow:start` → [conversation] → `/devflow:summarize` → Use CLEAR-optimized prompt
- **To implementation**: `/devflow:summarize` → `/devflow:plan` → `/devflow:implement` → `/devflow:archive`
- **Standalone use**: [Any conversation] → `/devflow:summarize` → Extract and optimize

**Related commands:**
- `/devflow:start` - Begin conversational exploration (typical previous step)
- `/devflow:plan` - Generate tasks from extracted mini-PRD (next step)
- `/devflow:fast` or `/devflow:deep` - Further optimize the extracted prompt
...
- Or inform user: "Our conversation was exploratory. I recommend `/devflow:start` to go deeper, or `/devflow:prd` for structured planning"
...
- Suggest starting fresh with `/devflow:prd` if major contradictions exist
