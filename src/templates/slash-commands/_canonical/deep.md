...
# DevFlow Deep Mode - Full CLEAR Framework Analysis
...
3. **Strategic Scope Detection** (before detailed analysis):
   ...
   **If 3+ strategic keywords detected**:
   ...
   - Switch to `/devflow:prd` for comprehensive strategic planning (recommended)
   - Continue with deep mode for prompt-level analysis only"

   **If user chooses to continue**, proceed with deep analysis but remind them at the end that `/devflow:prd` is available for strategic planning.
...
**Do NOT include (these belong in `/devflow:prd`):**
...
- **Fast mode** (`/devflow:fast`): C, L, E components - quick CLEAR cleanup
- **Deep mode** (`/devflow:deep`): Full CLEAR (C, L, E, A, R) - comprehensive analysis with alternatives and validation
- **PRD mode** (`/devflow:prd`): CLEAR-validated PRD generation - strategic planning with architecture decisions
...
**If user ran CLI command** (`devflow deep "prompt"`):
- Prompt is automatically saved ✓
- Skip to "Executing the Saved Prompt" section below

**If you are executing this slash command** (`/devflow:deep`):
- You MUST save the prompt manually
- Follow these steps:

#### Step 1: Create Directory Structure
```bash
mkdir -p .devflow/outputs/prompts/deep
```
...
- **Path**: `.devflow/outputs/prompts/deep/<prompt-id>.md`
...
Use the Write tool to update the index at `.devflow/outputs/prompts/deep/.index.json`:
...
Confirm:
- File exists at `.devflow/outputs/prompts/deep/<prompt-id>.md`
...
**Common workflows:**
- **Quick execute**: `/devflow:deep` → `/devflow:execute` → Implement
- **Review first**: `/devflow:deep` → `/devflow:prompts` → `/devflow:execute`
- **Thorough analysis**: `/devflow:deep` → Use optimized prompt + alternatives
- **Escalate to strategic**: `/devflow:deep` → (detects strategic scope) → `/devflow:prd` → Plan → Implement → Archive
- **From fast mode**: `/devflow:fast` → (suggests) `/devflow:deep` → Full analysis with A & R components

**Related commands:**
- `/devflow:execute` - Execute saved prompt
- `/devflow:prompts` - Manage saved prompts
- `/devflow:fast` - Quick CLEAR improvements (C, L, E only)
- `/devflow:prd` - Strategic PRD generation for architecture/business decisions
- `/devflow:start` - Conversational mode for exploring unclear requirements
...
- For architecture, security, and scalability, recommend `/devflow:prd`
...
```bash
echo '{"version":"1.0","prompts":[]}' > .devflow/outputs/prompts/deep/.index.json
```
...
- Ensure `.devflow/` directory is writable
...
- Remind at end that `/devflow:prd` is available for strategic planning
...
**Solution**:
- Switch to `/devflow:prd` for comprehensive planning
