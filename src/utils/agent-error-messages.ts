...
export class AgentErrorMessages {
  /**
   * Error: No PRD artifacts found in .devflow/outputs/
   * Used by: plan.ts, implement.ts
   */
  static noPrdFound(): string {
    return (
      'No PRD artifacts found in .devflow/outputs/\n\n' +
      'Agent recovery options:\n' +
      '  1. Execute /devflow:prd to generate comprehensive PRD\n' +
      '  2. Execute /devflow:summarize if conversation exists\n' +
      '  3. Use devflow plan --session <id> if saved session available\n\n' +
  ...
  static noTasksFound(projectName: string): string {
    return (
      ...
      'Agent recovery options:\n' +
      `  1. Execute: devflow plan --project ${projectName}\n` +
      '  2. Or execute /devflow:plan to generate interactively\n\n' +
      'After task generation completes, retry implementation.'
    );
  }

  /**
   * Error: .devflow-implement-config.json not found
   * Used by: task-complete.ts, implement workflow
   */
  static configNotFound(): string {
    return (
      'Configuration file not found: .devflow-implement-config.json\n\n' +
      'Agent recovery:\n' +
      '  1. Run: devflow implement\n' +
  ...
  static noArchivableProjects(): string {
    return (
      ...
      'Possible reasons:\n' +
      '  • No PRD projects in .devflow/outputs/\n' +
      '  • All projects already archived\n' +
      '  • No projects with completed tasks\n\n' +
      'Agent recovery options:\n' +
      '  1. Run: devflow archive --list to see archived projects\n' +
      '  2. Check: .devflow/outputs/ for active projects\n' +
      '  3. Or create new PRD with /devflow:prd'
    );
  }
  ...
  static cliExecutionFailed(command: string, exitCode: number, stderr?: string): string {
    ...
    return (
      ...
      '  2. Verify DevFlow is installed: devflow --version\n' +
