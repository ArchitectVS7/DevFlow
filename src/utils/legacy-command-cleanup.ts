...
export async function collectLegacyCommandFiles(
  ...
  for (const name of commandNames) {
    ...
    if (adapter.name === 'cline') {
      ...
      const oldCandidates = [
        ...
        path.join(oldDir, `devflow-${name}${extension}`),
      ];
    ...
    if (adapter.name === 'gemini' || adapter.name === 'qwen') {
      const namespaced = commandDir.endsWith(path.join('commands', 'devflow'));
  ...
  if (adapter.name === 'claude-code') {
    ...
    const colonFiles = await FileSystem.listFiles(commandsDir, /^devflow:.*\.md$/);
    for (const file of colonFiles) {
      legacyPaths.add(path.resolve(path.join(commandsDir, file)));
    }

    const devflowDir = path.resolve(commandsDir, 'devflow');
    const hyphenFiles = await FileSystem.listFiles(devflowDir, /^devflow-.*\.md$/);
    for (const file of hyphenFiles) {
      legacyPaths.add(path.resolve(path.join(devflowDir, file)));
    }
  }
